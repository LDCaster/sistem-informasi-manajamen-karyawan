...
@section('content')
    ...
    <div x-data="exportTable">
        <div class="panel flex items-center overflow-x-auto whitespace-nowrap p-3 text-primary">
            ...
            <h5 class="text-lg font-semibold dark:text-white-light">Pelatihan Karyawan</h5>
        </div>
        @if (session('success'))
            <!-- danger -->
        ...
        document.addEventListener("click", function(event) {
            if (event.target.closest("button[x-tooltip='Delete']")) {
                ...
                if (confirm("Apakah Anda yakin ingin menghapus data ini?")) {
                    fetch(`/pelatihan-karyawan/${karyawanId}`, {
                            method: "DELETE",
                            headers: {
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('exportTable', () => ({
                datatable: null,
                pelatihankaryawan: @json($pelatihankaryawan), // Pass the pelatihankaryawan data to Alpine.js
                init() {
                    ...
                    const dataPelatihan = this.pelatihankaryawan.map(pelatihan => {
                        ...
                        return [
                            '', // Kolom Action
                            pelatihan.id, // No
                            karyawan.nik || 'N/A', // NIK
                            karyawan.nip || 'N/A', // NIP
                            karyawan.nama || 'N/A', // Nama
                            pelatihan.nama_pelatihan, // Nama Pelatihan
                            pelatihan.tanggal_pelatihan, // Tanggal Pelatihan
                            pelatihan.file_pelatihan
                        ];
                    });
                    ...
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: ['<div class="text-center">Action</div>', 'No', 'NIK',
                                'NIP', 'Nama', 'Nama Pelatihan', 'Tanggal Pelatihan',
                                'File Pelatihan'
                            ],
                            data: dataPelatihan,
                        },
                        ...
                        columns: [{
                                ...
                                render: (data, cell, row) => {
                                    return `<div class="flex items-center">
                                                <a href="/pelatihan-karyawan/${row.cells[1].innerText}/edit/" class="ltr:mr-2 rtl:ml-2" x-tooltip="Edit">
                            ...
                            {
                                select: 7, // Kolom untuk File Pelatihan
                                render: (data, cell, row) => {
                                    return data ?
                                        `<a href="${data}" target="_blank" class="text-primary">Download</a>` :
                                        'No File';
                                },
                            },
                        ],
